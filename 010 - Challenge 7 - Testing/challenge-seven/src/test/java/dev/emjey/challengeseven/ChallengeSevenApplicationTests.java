package dev.emjey.challengeseven;


import com.fasterxml.jackson.databind.ObjectMapper;
import dev.emjey.challengeseven.pojo.Contact;
import dev.emjey.challengeseven.repository.ContactRepository;
...
// Project: contacts
// FileName: ContactsApplicationTests.java
// Date: 2023/12/15
...
class ContactsApplicationTests {
    ...
    private Contact[] contacts = new Contact[]{new Contact("1", "Jon Snow", "6135342524"), new Contact("2", "Tyrion Lannister", "4145433332"), new Contact("3", "The Hound", "3452125631"),};
    ...
    public void getContactByIdTest() throws Exception {
        ...
        mockMvc.perform(request).andExpect(status().is2xxSuccessful()).andExpect(content().contentType(MediaType.APPLICATION_JSON)).andExpect(jsonPath("$.name").value(contacts[0].getName())).andExpect(jsonPath("$.phoneNumber").value(contacts[0].getPhoneNumber()));
    ...
    public void getAllContactsTest() throws Exception {
        ...
        mockMvc.perform(request)
                ...
                .andExpect(jsonPath("$.[?(@.id == 1 && @.name ==\"Jon Snow\" && @.phoneNumber == \"6135342524\")]").exists())
    ...
    public void validContactCreation() throws Exception {
        RequestBuilder request = MockMvcRequestBuilders.post("/contact")
                ...
                .content(objectMapper.writeValueAsString(new Contact("EmJey", "8585")));
